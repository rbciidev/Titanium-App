...
function Home(parent){
	...
	var frameimg = Ti.UI.createImageView({
		image : '/images/pic-frame-iphone1.png',
	...
	var balancelbl = Ti.UI.createLabel({
		text: "Your current total balance is:",
		font : {fontSize : 12},
		left : xleft,
		top : 65
	});
	...
	*/
	...
	var headerFont = GetFont(14, 'bold');
	var hcolor = 'white';
	...
	function createRow(item){
		...
		msgview.addEventListener('doubletap', function(e){
			MainViewer = require('ui/common/MainViewer');
			var win = new MainViewer();
			win.webview.setHtml(msgview.html);
			win.open();
		});
		...
		msgview.addEventListener('load',function(e){
	...
	function reload(isscroll){
		...
		xhr.open('GET', url);
		var rdata; 
		if(ismemo == true) rdata = {id : pageno};
		else rdata =  {id : selectedMemberId, id2:pageno};
		xhr.send(rdata);
	...
	bbar.addEventListener('click',function(e){
		...
		memoview.animate({view:tableview, transition:Ti.UI.iPhone.AnimationStyle.FLIP_FROM_LEFT});
		if(e.index == 0 && !ismemo){
			ismemo = true;
			pageno = 1;
			reload();
