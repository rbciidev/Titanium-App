...
function Encashments(){
	...
	function initDestData(){
		...
		var xhr = Ti.Network.createHTTPClient({
			onload: function(){
				...
				destCombo = createComboBox(150,{fontSize:14, fontWeight:'bold'},'Select Destination', destData, destChangedHandler);
				destCombo.left = '2%';
				destCombo.top = '2%';
	...
	var withdrawBtn = createButton('2%', '2.3%', 150, 'Withdraw');
		
	function validateForm(){
		var amount = parseFloat(amountText.value.replace(',',''));
		var bal = parseFloat(balance.replace(',',''));
		var destcode = Ti.App.Properties.getString('destinationCode');
		Ti.API.info('amount: '+amount + '  balance: '+bal);
		if (amount > bal || amount <= 0 ||
			(countryCode == 'PHIL' && amount < 500)  ||
			(destcode == null || destcode.length <= 0)) 
			return false;
		else 
			return true;
	}	
	...
	withdrawBtn.addEventListener('click', function(){
		...
		if(isValid == true){
			...
			if(dest == null || dest.length <= 0)
				showMessage("Invalid Destination", "Please select a destination.");
			else if(countryCode == 'PHIL' && parseFloat(amountText.value.replace(',','')) < 500)
	...
	function accountBalanceHandler(){
		...
		accountCombo = createComboBox(150,{fontSize:14, fontWeight:'bold'},'Select a User Id', adata, selectionChangedHandler);
